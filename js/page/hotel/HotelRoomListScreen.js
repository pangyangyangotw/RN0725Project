...
let _this;
// @WrappShaowView()
class HotelRoomListScreen extends SuperView {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            AdvanceDayViolationMode:false,//提前天数是否可预订
            ...
            isVisible2:false,
            bedTypeGroup:[],
            // IsCustomerAgreement:this.params.IsCustomerAgreement
        }
        _this = this;
    }
    //展示详情弹出View
    _showTipView = (sectionArr, data, groupName) => {
        sectionArr.forEach(function (item) {
            if (item.HotelCode === data.HotelCode && item.RoomTypeCode === data.RoomTypeCode) {
                _this.setState({
                    imageArrs: item.RoomImageList,//房间图片
    ...
    _cityLocationQuery = () => {
        const { hotel, feeType, city, IsAgreement } = this.params;
        let model = {
            citycode: city.Code,
            latitude: hotel.Latitude,
            longitude: hotel.Longitude
        }
        HotelService.CityLocationQuery(model).then(response => {
    ...
    _loadList = (RatePlanId, RoomTypeId, index) => {
        const { hotel, feeType, city, IsAgreement,paymentArrival,paymentAdvance,roomCount,JourneyId,everyPerNum } = this.params;
        ...
        let model = RatePlanId && RoomTypeId ?//判断是初次进来还是点击预订时回来获取规则,这两个值存在是获取规则
            {
                ...
                // RoomCount: comp_checkTravellers && comp_checkTravellers.Travellers ? comp_checkTravellers.Travellers.length: comp_travelers && comp_travelers.Travellers.length,
                RoomCount:roomCount,
        ...
        HotelService.getHotelDetail(model).then(response => {
            ...
            if (response && response.success) {
                if (response.data) {
                    if (RatePlanId && RoomTypeId) {//判断是初次进来还是点击预订时回来获取规则
                        let value
                        ...
                        if (response.data.RoomGroups) {
                            response.data.RoomGroups.forEach(obj => {
                                ...
                                // let room =obj.Rooms ? obj.Rooms[0] : {};
                                this.state.roomList.push({
                                    ...
                                    Rooms: obj.Rooms,
                                    LowRate: obj.LowRate?obj.LowRate : (obj.RatePlans[0].TotalPrice)
                                });
                            })
                        }
                        //需求DD12002价格相同时，含有可积分（IsRewardPoint）的房型排在不含可积分前面
                        //如果this.state.roomList里的对象里waitData这个数组里有一个对象中IsRewardPoint为true的，就标记这个对象里的IsRewardPoint为true
                        this.state.roomList.forEach(item => {
                            if (item.waitData.some(_item => _item.IsRewardPoint)) {
                                item.IsRewardPoint = true;
                            }
                        })
                        //this.state.roomList数组里LowRate值重复的，标记有IsRewardPoint = true的排在没有的前面
                        this.state.roomList.sort((a, b) => {
                            const aReward = String(a?.IsRewardPoint || '');
                            const bReward = String(b?.IsRewardPoint || '');
                            ...
                            return Number(a.LowRate || 0) - Number(b.LowRate || 0);
                        })
                        ...
                        let bedTypeGroup = [];
                        response?.data?.RoomGroups?.map(obj => {
                            obj.RatePlans?.map(item => {
    ...
    _getNewRules = (RatePlanId, RoomTypeId, isAlert, Channel, SubChannel, callBack) => {
        const { hotel, feeType, city, IsAgreement,roomCount, everyPerNum } = this.params;
        const { selectDate, longDay } = this.state;
        const { comp_checkTravellers, comp_travelers, loadHotelCanselRule,apply } = this.props;
        let liveDate = selectDate.addDays(longDay);
        let journeyid = 0;
        ...
        HotelService.getHotelDetail(model).then(response => {
            ...
            if (response && response.success && response.data) {
                if (isAlert) {
                    ...
                    //  if(response.data.RoomGroups&&response.data.Rooms && response.data.Rooms[0] && response.data.Rooms[0].RatePlans &&  response.data.Rooms[0].RatePlans[0])
                    // callBack && callBack(response.data.Rooms[0].RatePlans[0]);
                    callBack && callBack();
                }
    ...
    _showRuls = (data) => {
        if (data.CancelRules && data.CancelRules[0].Cancelable === 4) {
            // this._loadList(data.RatePlanCode, data.RoomTypeCode, 1);
    ...
    _hotelDetailBtnClick = () => {
        ...
        let liveDate = selectDate.addDays(longDay);
        this.push('HotelInstrotuction', { ...this.state.roomDetail, selectDate: selectDate.format('yyyy-MM-dd hh:mm'), liveDate: liveDate.format('yyyy-MM-dd hh:mm'),RoomCount:roomCount });
    }

    _orderBtnClick = (item, section, BigPrice) => {
        const { selectDate, longDay, city, feeType,roomCount, everyPerNum} = this.params;
        let liveDate = selectDate.addDays(longDay);
        let model = {
        ...
        HotelService.HotelOrderRatePlanPriceCheck(model).then(response => {
            ...
            if (response && response.success && response.data && response.data.data) {
                ...
                this.toastMsg(response.message || '预订失败');
            }
        }).catch(error => {
            this.hideLoadingView();
   ...
   _orderBtnClickNext = (item, section, BigPrice)=>{
    ...
    const { ViolationModeCity, LimitPrice, roomDetail,customerInfo,ShareRoomApplyFlag } = this.state;
    const { compSwitch, comp_checkTravellers, comp_travelers, loadHotelCanselRule } = this.props;
    loadHotelCanselRule('');
    let roomIdModel = section.room;

    // this._loadList(item.RatePlanCode, item.RoomTypeCode);
            ...
            if (response && response.success) {
                if (response.data) {
                    if(customerInfo.Setting.HotelRulesVerifyType==0){//均价判断是否超标
                            ...
                            } else if (response.data.ViolationMode != 1 ) {//判断是不是去提示页状态
                                if ((LimitPrice!=0 && LimitPrice < item.AvgPrice) || response.data.StarRateLimit || response.data.AdvanceDayLimit) {
                                    this._clickRules(response, model,item);
                                } else {//均价未超标
                                    compSwitch ?
                                        this.push('Hotel_comp_CreateOrderScreen', {model, item, JourneyId})
                                        :
                                        this.push('HotelOrder', {model, item, JourneyId});
                                }
                            } else {//超标审批、超标现付
                                compSwitch ?
                                    this.push('Hotel_comp_CreateOrderScreen', {model, item, JourneyId})
                        ...
                        if(LimitPrice!=0 && LimitPrice < BigPrice){//单价超标 去提示页2
                            this._clickRules(response,model);
                        }else{//单价未超标
                            this.push('HotelOrder', {model,JourneyId});
                        }
                    }
                }
            } else {
    ...
    _clickRules = (response, model, item) => {
        if (response.code == 0) {
            ...
            this.push('HotelRule', {model, item, JourneyId:this.params.JourneyId});
    ...
    _toMap = () => {
        ...
        if(!hotel.Latitude){
            hotel.Latitude = hotel.Lat
            hotel.Longitude = hotel.Lng
        }
        this.showLoadingView();
    ...
    renderBody() {
        const { roomList, roomListArrs, Loading,itemData } = this.state;
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        {this.state.itemData && <Animated.View style={{ position: "absolute", top: this.state.securetyTipViewY, height: screenHeight, width: screenWidth }}>
                            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(3,3,3,0.3)' }} >
                                <TouchableOpacity style={{position: "absolute",height: screenHeight / 3, width: screenWidth, top: 0}} onPress={this._hiddenTipView}/>
                                    ...
                                    <ScrollView showsVerticalScrollIndicator={false} style={{ marginBottom: 40 }} keyboardShouldPersistTaps='handled'>
                                                            ...
                                                            <TouchableOpacity style={{ width: screenWidth - 30, height: 180 }}
                                                                ...
                                                                >
                                                                    <View style={styles.imageItemStyle}>
                                                                        ...
                                                                        <Text style={{ color: '#fff',fontSize:12 }}>{index + 1 + '/' + this.state.imageArrs.length}</Text>
                                                                    </View>
                                                                </ImageBackground>
                                        ...
                                        <View style={{ width: screenWidth - 30 }}>
                                            ...
                                            <View style={{ flexDirection: 'row', width: screenWidth - 30 }}>
                                                {this.state.itemData.RatePlanName ? <View style={{ width: screenWidth / 2 - 30, flexDirection: 'row', marginBottom: 15, marginTop: 20 }}>
                                                    ...
                                                    <CustomText text={': ' + this.state.itemData.RatePlanName} style={{ fontSize: 14 }} />
                                                </View> : null}
                                            </View>
                                            <View style={{ marginTop: this.state.itemData.RatePlanName ? 0 : 20, width: screenWidth - 30, flexDirection: "row", flexWrap: 'wrap' }}>

                                                {this.state.itemData.BedType ? <View style={styles.textViewSt}>
                                                    <CustomText text='床型' style={styles.textSt} />
                                                    <CustomText text={': ' + this.state.itemData.BedType} style={{ fontSize: 14 }} />
                                                </View> : null}
                                                {this.state.itemData.MealTypeDesc ? <View style={styles.textViewSt}>
                                                    <CustomText text='早餐' style={styles.textSt} />
                                                    {
                                                        this.state.itemData.MealType==15? 
                                                        <AntDesign name={'questioncircle'} color={Theme.theme} size={18} style={{ marginHorizontal: 5 }} onPress={this._showAlert.bind(this)} />
                                                        :<CustomText text={': ' + this.state.itemData.MealTypeDesc} style={{ fontSize: 14 }} />
                                                    }
                                                </View> : null}
                                                {this.state.itemWindow ? <View style={styles.textViewSt}>
                                                    <CustomText text='是否有窗' style={styles.textSt} />
                                                    <CustomText text={': ' + this.state.itemWindow} style={{ fontSize: 14 }} />
                                                </View> : null}
                                                {this.state.itemArea ? <View style={styles.textViewSt}>
                                                    <CustomText text='' style={styles.textSt} />
                                                    <CustomText text={'㎡: ' + this.state.itemArea} style={{ fontSize: 14, color: Theme.darkColor }} />
                                                </View> : null}
                                                {this.state.itemFloor ? <View style={styles.textViewSt}>
                                                    <CustomText text='楼层' style={styles.textSt} />
                                                    <CustomText text={': ' + this.state.itemFloor} style={{ fontSize: 14 }} />
                                                </View> : null}
                                                {this.state.itemData.ChannelTag ? <View style={styles.textViewSt}>
                                                    <CustomText text='所属渠道' style={styles.textSt} />
                                                    <CustomText text={': ' + this.state.itemData.ChannelTag} style={{ fontSize: 14 }} />
                                                </View> : null}
                                                ...
                                                {this.state.itemData.PaymentType ? <View style={styles.textViewSt}>
                                                    ...
                                                    <CustomText text=':' style={styles.textSt} />
                                                    <CustomText text={this.state.itemData.PaymentType === 1 ? '到付' : '预付'} style={{ fontSize: 14 }} />
                                                </View> : null}
                                        ...
                                        {this.state.itemDescription ? <View>
                                            <View style={{flexDirection:'row'}}>
                                                <CustomText text='房间简介' style={{ fontSize: 14, color: Theme.darkColor }} />
                                                <CustomText text='：' style={{ fontSize: 14, color: Theme.darkColor }} />
                                            </View>
                                            <CustomText text={this.state.itemDescription} style={{ fontSize: 14 }} />

                                        </View> : null}
                                        {itemData?.BookingRules?.[0]?.Desc ? <View>
                                            <View style={{flexDirection:'row'}}>
                                                <CustomText text='房价描述' style={{ fontSize: 14, color: Theme.darkColor }} />
                                                <CustomText text='：' style={{ fontSize: 14, color: Theme.darkColor }} />
                                            </View>
                                            <CustomText text={itemData.BookingRules[0].Desc} style={{ fontSize: 14 }} />
                                        </View> : null}
                                    </ScrollView>
                                </View>
                            </View>
        ...
        return (
            <TouchableHighlight underlayColor='transparent' onPress={this._sectionclick.bind(this, section)}>
                <View style={{ flexDirection: 'row', backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: Theme.lineColor, padding: 10}}>
                    <Image style={{ marginLeft: 5, marginTop: 1, width: 80, height: 80, borderRadius: 4, }} source={room.CoverImage ? { uri: room.CoverImage } : require('../../res/image/replaceFcm.jpeg')} />
                    <View style={{}}>
                        <View style={{width:global.screenWidth-120,marginLeft:10 }}>
                            <CustomText text={room.RoomName} numberOfLines={2} />
                            <CustomText text={room.Desc} numberOfLines={3} style={{ marginTop: 5, color: Theme.aidFontColor}} />
                        </View>
                        <View style={{ flexDirection: 'row-reverse', alignItems: 'center',width:global.screenWidth-110, }}>
                            <AntDesign name={section.isOpen ? 'upcircleo' : 'downcircleo'} size={16} color={Theme.theme}  />
                            <CustomText text={`${section.data?.[0]?.AvgPrice}`} style={{ color: Theme.theme, marginRight: 5,fontSize:20 }} />
                            <CustomText text={`¥`} style={{ color: Theme.theme, marginTop: 4,fontSize:14,fontWeight:'bold' }} />
                        </View>
                        ...
                        <View style={{ flexDirection: 'row',paddingHorizontal:10 }}>
                            <View style={{ flex: 1, marginRight: 10 }}>
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 5 }}>
                                    ...
                                    <CustomText style={{ marginLeft: 3, fontSize: 13, color: Theme.darkColor }} text={data.StatusDesc} />
                                </View>
                                ...
                                <TouchableOpacity style={{ flexDirection:'row',marginBottom:15,width:170,flexWrap:'wrap' }} onPress={this._showRuls.bind(this, data)} >
                                            ...
                                            />
                                            <CustomText style={{ marginLeft: 5, fontSize: 13, color: Theme.theme }} text={data.IsRewardPoint === true ? '可积分' : data.IsRewardPoint === false ? '不可积分' : ''}></CustomText>
                                </TouchableOpacity>
        ...
        return (
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
             {!isOnlyApply?
                ...
                >
                    <View style={{ flexDirection: 'column',alignItems:'flex-end',borderRadius:2 }}>
                        <View style={{flexDirection:'row'}}>
                            <Text allowFontScaling={false} style={{  marginTop: 10, color: Theme.theme, fontSize: 18, fontWeight: 'bold' }}>{'¥' + data.AvgPrice}</Text>
                            {data.CurrencyInfo?.PriceList?.AvgTax ? <CustomText text='(含税)' style={{ fontSize: 12, color: Theme.theme,marginRight: 10,marginTop:15,marginRight: 10, }} />:null}
                        </View>
                        {data.AvgTax?<View style={{flexDirection:'row',marginRight: 10}}>
                            <CustomText text='税费' style={{ fontSize: 12, color: Theme.theme}} />
                            <CustomText text={'¥'} style={{fontSize:12, fontWeight: 'bold', color: Theme.theme,marginLeft:3}}></CustomText>
                            <CustomText text={data.AvgTax} style={{ fontSize: 12, color: Theme.theme,marginLeft:3}} />
                        </View>:null}
                        {
                            Util.Parse.isChinese() ?
                                StarRateLimit || AdvanceDayLimit ||(this.state.LimitPrice!=0 && this.state.LimitPrice < data.AvgPrice) ?
                                <TouchableOpacity style={{borderRadius:2, backgroundColor:Theme.redColor,marginRight:10}} onPress={()=>this._alert(StarRateLimit,AdvanceDayLimit,this.state.LimitPrice!=0 && this.state.LimitPrice < data.AvgPrice)}>
                    ...
                    <View style={{   borderRadius: 4 }}>
                        <View style={{
                            ...
                            <CustomText text={data.Status === 3 ? '订 完' : '预 订'} style={{ color: '#fff', fontSize: 14 }} />
                        </View>
                        <View style={{
                            flex: 2,
                            paddingHorizontal: 5,
                        ...
                        {Util.Parse.isChinese() ? null :
                            <View style={{ alignItems: 'center', justifyContent: 'center',borderRadius:2}}>
                                {    
                                    StarRateLimit || AdvanceDayLimit ||(this.state.LimitPrice!=0 && this.state.LimitPrice < data.AvgPrice) ?
                                    <TouchableOpacity style={{borderRadius:2, backgroundColor:Theme.redColor, marginRight:10}} onPress={()=>this._alert(StarRateLimit,AdvanceDayLimit,this.state.LimitPrice!=0 && this.state.LimitPrice < data.AvgPrice)}>
                                        <CustomText text='违背' style={styles.ruleStyle} />
               ...
               <TouchableOpacity style={{ marginLeft: 10, flexDirection: 'row', alignItems: 'center' }}
                ...
                >
                    <View style={{ flexDirection: 'column',alignItems:'flex-end',borderRadius:2 }}>
                        {/* <Text allowFontScaling={false} style={{ marginRight: 10, marginTop: 10, color: Theme.theme, fontSize: 18, fontWeight: 'bold' }}>{'¥' + data.AvgPrice}</Text> */}
                        <View style={{flexDirection:'row'}}>
                            <Text allowFontScaling={false} style={{  marginTop: 10, color: Theme.theme, fontSize: 18, fontWeight: 'bold' }}>{'¥' + data.AvgPrice}</Text>
                            {data.CurrencyInfo?.PriceList?.AvgTax ? <CustomText text='(含税)' style={{ fontSize: 12, color: Theme.theme,marginRight: 10,marginTop:15,marginRight: 10, }} />:null}
                        </View>
            ...
            <View style={{ justifyContent: 'center', alignItems: 'center' }}>
               { 
                ...
                <TouchableOpacity style={{ marginLeft: 10, flexDirection: 'row', alignItems: 'center' }}
                    ...
                    <View style={{ flexDirection: 'column',alignItems:'flex-end',borderRadius:6 }}>
                        <View style={{flexDirection:'row',marginTop: 10,}}>
                        <CustomText text={'¥'} style={{fontSize:14, marginTop:5, fontWeight: 'bold', color: Theme.theme}}></CustomText>
                        <Text allowFontScaling={false} style={{ color: Theme.theme, fontSize: 20 }}>{data.AvgPrice}</Text>
                        {data.CurrencyInfo?.PriceList?.AvgTax ? <CustomText text='(含税)' style={{ fontSize: 12, color: Theme.theme,marginRight: 10,marginTop:5 }} />:null}
                        </View>
                        {data.AvgTax?<View style={{flexDirection:'row',marginRight: 10}}>
                            <CustomText text='税费' style={{ fontSize: 12, color: Theme.theme}} />
                            <CustomText text={'¥'} style={{fontSize:12, fontWeight: 'bold', color: Theme.theme,marginLeft:3}}></CustomText>
                        ...
                        {
                          StarRateLimit || AdvanceDayLimit ||(this.state.LimitPrice < data.AvgPrice && this.state.LimitPrice != 0) ?
                            <TouchableOpacity style={{borderRadius:2, backgroundColor:Theme.redColor, marginRight:10}} onPress={()=>this._alert(StarRateLimit,AdvanceDayLimit,this.state.LimitPrice < data.AvgPrice)}>
                                <CustomText text='违背' style={styles.ruleStyle} />
                            </TouchableOpacity>:null
                        }
                    </View>
    ...
    _renderHeader = () => {
        ...
        const { selectDate, longDay, location, isVisible, isVisible2,roomDetail,bedTypeGroup } = this.state;
        let liveDate = selectDate.addDays(longDay);
        let imageArr = [];
        let allImageArr = [];
        let imageNum = 0
        ...
        return (
            <View style={{backgroundColor:Theme.normalBg}}>
                                ...
                                >
                                    ...
                                    >
                                        <View style={styles.imageStyle}>
                                            {/* <Ionicons name={'ios-images'} size={14} style={{ color: '#fff' }} /> */}
                                            <Image source={require('../../res/Uimage/hotelFloder/_pic.png')} style={{ width: 14, height: 14 }} />
                                            <Text style={{ color: '#fff', marginLeft: 2, fontSize: 12 }}>{index+1}/</Text>
                ...
                <View style={{ backgroundColor: '#fff', marginTop: -15, borderTopRightRadius: 15, borderTopLeftRadius: 15 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', margin: 15, marginTop: 25, marginBottom: 10, justifyContent: 'space-between' }}>
                        <View style={{flex:1}}>
                            <View style={{  alignItems: 'baseline' }}>
                                <CustomText text={Util.Parse.isChinese()?hotel.HotelName:hotel.HotelNameEn} style={{ fontSize: 16, fontWeight: 'bold' }} ellipsizeMode='tail' />
                                {
                                  starArr.length>0?
                                    <View style={{ flexDirection: 'row' }}>
                                        {  
                            ...
                            {/* {hotel.HotelNameEn || this.state.roomDetail&&this.state.roomDetail.HotelNameEn ? <CustomText text={'(' + (hotel.HotelNameEn || this.state.roomDetail.HotelNameEn)+ ')'} style={{ fontSize: 13, color: 'gray',flex:1 }} ellipsizeMode='tail' /> : null} */}
                            {this.state.roomDetail && this.state.roomDetail.OpenDate ?
                                <View style={{ display:'flex', flexDirection: 'row' ,alignItems:"center"}}>
                                     <CustomText text={'开业时间'} />
                                     <CustomText text={'：'} />
                                     <CustomText text={ this.state.roomDetail.OpenDate} style={{ fontSize: 13, color: Theme.aidFontColor}} />
                                </View>:null
                            }
                        </View>
                        <TouchableOpacity style={{ height: 30, alignItems: 'center', flexDirection: 'row' ,marginLeft:10}}
                    ...
                    {/* {
                        hotel.CommentScore ?
                            <View style={{ flexDirection: 'row', marginLeft: 15, alignItems: 'baseline' }}>
                                <CustomText style={{ color: Theme.theme, fontSize: 13 }} text='好评率' />
                                <CustomText style={{ color: Theme.theme, fontSize: 13 }} text=':' />
                                <CustomText text={(hotel.CommentScore * 100) + '%'} style={{ fontSize: 14, color: Theme.theme }} />
                    ...
                    <TouchableOpacity style={{ marginTop: 8}} onPress={this._toMap}>
                        <ImageBackground source={require('../../res/Uimage/hotelFloder/_mapBg.png')}
                            style={{ marginHorizontal:10, justifyContent:'space-between',flexDirection:'row',paddingHorizontal:10,alignItems:'center',height:64}} imageStyle={{ borderRadius:4 }}>
                             <View style={{width:270}}>
                                <CustomText numberOfLines={2} text={ this.state.roomDetail && this.state.roomDetail.Address ? this.state.roomDetail.Address : hotel.Address ? hotel.Address : null } 
                                            style={{ fontSize: 13,}} 
                                /> 
                    ...
                    <View style={{marginVertical:6}}>
                        <View style={styles.dateStyle}>
                            ...
                            <View style={{ height: 0.5, width: 80, backgroundColor: Theme.theme }} />
                        ...
                        <View style={{ flexDirection: 'row', paddingLeft: 15, flexWrap: 'wrap',paddingVertical:15,borderBottomWidth:6,borderColor:Theme.normalBg, alignItems: 'center' }}>
                            <TouchableOpacity style={{ minWidth: Util.Parse.isChinese() ? screenWidth / 6 : null, height: 24, alignItems: 'center', justifyContent: 'center', borderWidth: 0.7, borderRadius: 3, borderColor: this.state.selectEligibility ? Theme.theme : 'gray',backgroundColor:this.state.selectEligibility ? Theme.greenBg : '#fff', marginVertical:5, marginRight: 5, padding: 4 }}
                                ...
                                <CustomText text='符合城市标准' style={{ fontSize: 13, color: this.state.selectEligibility ? Theme.theme : Theme.fontColor }} />
                            </TouchableOpacity>
                            <TouchableOpacity style={{ minWidth: Util.Parse.isChinese() ? screenWidth / 6 : null, height: 24, alignItems: 'center', justifyContent: 'center', borderWidth: 0.7, borderRadius: 3, borderColor: this.state.selectAgreement ? Theme.theme : 'gray',backgroundColor:this.state.selectAgreement ? Theme.greenBg : '#fff', marginVertical:5, marginRight: 5, padding: 4 }}
                                ...
                                <CustomText text='协议价' style={{ fontSize: 13, color: this.state.selectAgreement ? Theme.theme : Theme.fontColor }} />
                            </TouchableOpacity>
                            <TouchableOpacity style={{ minWidth: Util.Parse.isChinese() ? screenWidth / 6 : null, height: 24, alignItems: 'center', justifyContent: 'center', borderWidth: 0.7, borderRadius: 3, borderColor: this.state.selectBreakfast ? Theme.theme : 'gray',backgroundColor:this.state.selectBreakfast ? Theme.greenBg : '#fff', marginVertical:5, marginRight: 5, padding: 4}}
                                ...
                                <CustomText text='含早餐' style={{ fontSize: 13, color: this.state.selectBreakfast ? Theme.theme : Theme.fontColor }} />
                            </TouchableOpacity>
                            ...
                            <Popover
                                ...
                                from={(sourceRef, showPopover) => (
                                    ...
                                    <TouchableOpacity style={{ minWidth: Util.Parse.isChinese() ? screenWidth / 6 : null, height: 24, alignItems: 'center', justifyContent: 'center', borderWidth: 0.7, borderRadius: 3, borderColor: this.state.paySwich ? Theme.theme : 'gray',backgroundColor:this.state.paySwich ? Theme.greenBg : '#fff', marginRight: 5, padding: 4 }}
                                        onPress={
                                           ()=>{
                                            this.setState({
                                    ...
                                    ><CustomText text={this.state.paySwich ? this.state.paySwich : '支付方式'} style={{ fontSize: 13, color: this.state.paySwich ? Theme.theme : Theme.fontColor }} /></TouchableOpacity>
                            ...
                            <Popover
                                ...
                                from={(sourceRef, showPopover) => (
                                    <TouchableOpacity style={{ minWidth: Util.Parse.isChinese() ? screenWidth / 6 : null, height: 24, alignItems: 'center', justifyContent: 'center', borderWidth: 0.7, borderRadius: 3, borderColor: this.state.bedSwich ? Theme.theme : 'gray',backgroundColor:this.state.bedSwich ? Theme.greenBg : '#fff', padding: 4 }}
                                        onPress={
                                            ()=>{
                                                this.setState({
                                    ...
                                    ><CustomText text={this.state.bedSwich ? this.state.bedSwich : '床型'} style={{ fontSize: 13, color: this.state.bedSwich ? Theme.theme : Theme.fontColor }} /></TouchableOpacity>
                                )}>
                                <ScrollView 
        ...
        return (
            <View>
                <View style={{ height: 200, justifyContent: 'center', alignItems: 'center' }}>
                    <CustomText text={roomListArrs.length == 0 && this.state.OnOff ? '没有符合您查询条件的内容' : ''} style={{ color: Theme.darkColor }} />
                    {/* <Text style={{color:Theme.darkColor}}>{}</Text> */}
                </View>
            </View>
        )
    }
        ...
        let arrs = roomListCopy.map((items, i) => {
            ...
            if (this.state.bedSwich) {//筛选房型
                newarr = newarr.filter(item => {
                    ...
                    //筛选房型与选中相同的
                    return item?.BedTypeGroup === this.state.bedSwich
                })
            }
             if(newarr.length > 0){
                  let lowPrice;
...
export default connect(getStatePorps, getAction)(HotelRoomListScreen);
const styles = StyleSheet.create({
    ...
    ruleStyle: { fontSize: 12,color:'#fff',paddingHorizontal:5,paddingVertical:1},
