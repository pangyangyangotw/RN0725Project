...
class Hotel_comp_CreateOrderScreen extends SuperView {
                ...
                Pop.show(
                    <View style={styles.alertStyle}>
                       ...
                       <View style={{width:'100%'}}>
                        ...
                        //         })
                        //        :
                        response.data.RuleDesc&&response.data.RuleDesc.map((item, index)=>{
                                return(
                                  <View style={{flexDirection:'row',padding:2}} key={index}>
    ...
    _compOrderBtnClick = (totalPrice) => {
               ...
               if (alertWarning) return;
                ...
                if (obj && obj.Name) {
                    ...
                    let model = {
                        ...
                        IsVip: obj.IsVip,
                        Nationality:obj.NationalCode?obj.NationalCode: obj.Nationality,
        ...
        let newTravellers;
        if (this._isVendorCodeTVP()) {
            newTravellers = OrderSureTravellerList.map(obj => ({
                ...obj,
                Name: `${obj.Surname}/${obj.GivenName}`
            }));
        } else {
            newTravellers = OrderSureTravellerList;
        }
        //合并添加的同住人
        newTravellers = [...newTravellers,...addpersons];
        ...
        let params = Object.assign({ requestModel, AttachmentModel, from: 'hotel', Travellers: newTravellers ,totalPrice:totalPrice}, this.params, this.state);
    ...
    renderBody() {
        const {hotelCanselRule} = this.props;
        const { userInfo, LasterLiveTime,ServiceFeesData,travellers,paymentDesc,AdditionInfo,ApproveOrigin,customerInfo,roomCount,fileList,merchantPrice,PdfDictList,addPersonArr} = this.state;
        ...
        return (
            <LinearGradient style={{ flex: 1, position: 'relative' }} start={{x: 1, y: 0}} end={{x: 1, y: 0.5}} colors={[Theme.theme,Theme.normalBg]}>
                ...
                <KeyboardAwareScrollView keyboardShouldPersistTaps="handled" style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
                    ...
                    <View style={{marginHorizontal:10,borderRadius:6,padding:10,backgroundColor:'#fff',marginTop:10}}>
                        {
                    ...
                    <View style={{backgroundColor:'white',marginHorizontal:10,borderRadius:6,paddingHorizontal:10}}>
                        <View style={{height: 44,alignItems:'center',flexDirection: 'row',justifyContent: 'space-between',marginHorizontal:10}}>
                        ...
                        <View style={{ backgroundColor: 'white', padding: 10,justifyContent:'space-between',marginHorizontal:10,borderRadius:6}}>
                            <CustomText text={'如您的到店时间晚于18:00，为保证您的房间，请联系酒店进行信用卡担保。未担保的预定，酒店有权根据当天入住情况取消未担保的酒店预定。'} style={{fontSize:11, color:Theme.redColor}} ></CustomText>
                        </View>:null
                        }
                    </View>
                    {/* <DepartView
                                ...
                                <View style={{ justifyContent: 'center' }}>
                                   {  
                                        <View style={{flexDirection:'row'}} >
                                            ...
                                            {shareAllArr&&shareAllArr?.[i]?.length>1?null:
                                    ...
                                    }
                                    {
                                        shareAllArr&&shareAllArr[i]&&shareAllArr[i].map((item,index)=>{
                                           ...
                                           item.RoomNumber = i+1;
                                           return (
                                              <TouchableOpacity style={{flexDirection: 'row',marginLeft:10,width:global.screenWidth, alignItems:'center' }} onPress={()=>{this._toClickEdit(item,i,index)}}>
                                                    <TouchableHighlight onPress={()=>{this._deleteSharePerson(item,i,index)}} style={{ marginRight:10}} underlayColor='transparent'>
                                                        <AntDesign name={'delete'} size={20} color={Theme.theme} />
                                                    </TouchableHighlight>
                                                    {
                                                      this._isVendorCodeTVP() || (!showName && !item.selectCn) ?
    ...
    _toClickEdit = (data,j,index) => {
        ...
        if(!data.NationalName && data.Certificate){
            data.NationalName = data.Certificate.NationalName
            data.NationalCode = data.Certificate.NationalCode
        }
        if(data.IsTempCustomer){
            this.push('HotelAddPersonEditScreen',{
                ...
                VendorCodeTVP: this._isVendorCodeTVP(),
                callBack:(reason, i)=>{
                    if (!reason || !shareAllArr?.[j]?.[index]) return;
            ...
            this.push('HotelCompEditPassengerScreen', {
                customerInfo: customerInfo,
                passenger: data,
                ...
                callBack: (reason, i) => {
                    if (!reason || !shareAllArr?.[j]?.[index]) return;
                    travellers?.forEach((item, indexx) => {
                        ...
                        return (
                            <View key={index} style={{ marginHorizontal:10}}>
                                <View style={{ backgroundColor: 'white', borderBottomColor: Theme.lineColor, borderBottomWidth: 1, }}>
                                    <View style={{ justifyContent: 'center' }}>
                                        <Text style={{ }}>{I18nUtil.translate('房间1')}{parseInt(index) + 1}</Text>
                                    </View>
                                    <View style={{ flex: 1 }}>
                                        {/* {
                                            this._compAbloutLiveNumber(item,index)
                                        } */}
                                        {
                                            this._abloutLiveNumber(item,index)
                                        }
                                    </View>
                                </View>
                                <TouchableOpacity onPress={()=>{
                                    this._addSharePerson(index)
                                }}>
                                    <Text style={{ paddingVertical: 10, color:Theme.theme }}>{I18nUtil.translate('添加入住人')}</Text>
                                </TouchableOpacity>
                            </View>
                        )
                    })
        ...
        return (
            <View>
                <View  style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12 }}>
                    ...
                    {
                        // roomModel.IsNeedIDCard==true?
                            <TouchableOpacity 
                                onPress={()=>{
                                    this._EditClick(data)
                                }} 
                            ...
                            >
                                {
                                   this._isVendorCodeTVP() || (!showName && !data.selectCn) ?
                                    ...
                                    :<CustomText text={data.Name} style={{paddingVertical:6}}/>
                                }
    ...
    _EditClick = (data) => {
        const { roomModel } = this.params;
        const { travellers,customerInfo } = this.state;
        this.push('HotelCompEditPassengerScreen',{
            passenger:data,
            customerInfo:customerInfo,
            from:'comp_hotel',
    ...
    _renderBottomView = () => {
        ...
        if(ApplyTravelRule){
            ...
            if ((roomModel.PaymentType != 1) && RcModel?.ViolationMode === 3 && (PriceLimit * liveDay * roomCount) < (roomModel.AvgPrice * liveDay * roomCount + servicePrice)) {
                if(roomModel.PaymentType == 2 && roomModel.NeedCreditCard){
                    peronalCost = 0
                }else{
                    peronalCost = (parseFloat(roomModel.AvgPrice) - PriceLimit) * liveDay * roomCount;
                }
        ...
        return (
            <View style={{ height: 50, flexDirection: 'row', backgroundColor: 'white', alignItems: 'center',borderTopWidth:1 ,borderColor:Theme.lineColor}}>
                ...
                <View style={{ flex: 1, justifyContent: 'flex-end', flexDirection: 'row', alignItems: 'center' }}>
                    ...
                    <TouchableHighlight underlayColor='transparent' onPress={()=>{this._compOrderBtnClick(totalPrice)}}>
...
export default connect(getStatePorps,mapDispatchToProps)(Hotel_comp_CreateOrderScreen);
