...
class Inf_CompPassnegerView extends React.Component {
    ...
    _editPassengerRowClick = (index, type) => {
        const { employees, travellers, from, goFlightData,customerInfo,userInfo } = this.props;
        let data = null;
        if (type === 1) {
        ...
        NavigationUtils.push(this.props.navigation, fromTo, {
            ...
            userInfo:userInfo,
            DestinationNationalCode: this.props.DestinationNationalCode,
            DepartureNationalCode: this.props.DepartureNationalCode, 
             callBack: (obj) => {
    ...
    _renderPassneger = (passengers,type) => {
        ...
        return (
            passengers&&passengers.map((data, index) => {
                ...
                return (
                    <View key={index} style={{ backgroundColor: 'white',borderRadius:10 }}>
                        <TouchableHighlight underlayColor='transparent' onPress={this._editPassengerRowClick.bind(this, index, type)}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, borderTopColor: Theme.lineColor, borderTopWidth: 1 }}>
                                {/* {from !== 'car' ? <TouchableHighlight underlayColor='transparent' onPress={this._deletePasserngerClick.bind(this, index, type)}>
                                    ...
                                    </View>
                                    {
                                        data.CertificateType?
                                            <View style={{ flexDirection: 'row',flexWrap:'wrap' }}>
                                                ...
                                                <CustomText style={{ marginTop: 5, marginBottom: 5 }} text={':'} />
                                                <CustomText style={{ marginTop: 5, marginBottom: 5,marginLeft:6 }} text={Utils.Read.simpleReplace(data.CertificateNumber)} />
                                            </View>
                                        :
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 35 }}>
                                                ...
                                                <View style={{ flex: 1, alignItems: 'flex-end', justifyContent: 'center', paddingRight: 10 }}>
                                                    <MaterialIcons
                                                        ...
                                                        size={20}
                                                        color={item.show ?Theme.theme:Theme.darkColor}
                                                    />
                                                </View>
                                            </View>
                                        </View>
...
// export default withNavigation(Inf_CompPassnegerView);
export default function(props) {
    const navigation = useNavigation();
    return <Inf_CompPassnegerView {...props} navigation={navigation} />
}
