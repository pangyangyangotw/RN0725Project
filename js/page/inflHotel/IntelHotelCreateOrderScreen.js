...
class IntelHotelCreateOrderScreen extends SuperView {
    ...
    constructor(props) {
        ...
        this.state = {
             ...
             *  预计到店时间 
             */
            LasterLiveTime: Util.Date.toDate(this.params.checkIndate&&this.params.checkIndate.format('yyyy-MM-dd') + ' 14:00' ),
            /**
             * 入住间数
             */
            ...
            shareAllArr:this.props.shareAllArr?this.props.shareAllArr:share_AllArr,//合住人大数组
                ...
                Pop.show(
                    <View style={styles.alertStyle}>
                       ...
                       <View style={{width:'100%'}}>
                           ...
                           {
                               response.data.RuleDesc&&response.data.RuleDesc.map((item)=>{
                                   return(
                                     <View style={{flexDirection:'row',padding:2}}>
                                        <CustomText text={item&&item.Name+': '+item&&item.Desc}/>
                                     </View>
                                   )
                               })
                           }
    ...
    componentDidMount() {
        const { Contact, ApproveOrigin, travellers,roomCount,userInfo,AdditionInfo } = this.state;
        const { roomModel } = this.params;
        const { comp_userInfo} = this.props;
        let list = [];
        let travellersList = comp_userInfo.employees&&comp_userInfo.employees.concat(comp_userInfo.travellers);
    ...
    _orderBtnClick = (totalPrice) => {
        ...
        const { comp_userInfo, apply } = this.props;
            ...
            var visit = function (id) {
                ...
                if (nextId) {
                    var parentItem = dictItemList && dictItemList.find(function (it) {
                        ...
                        if (it.DictId == id) return true;
                        if (it.DictName && cfg && cfg.Name && it.DictName == cfg.Name) return true;
                        return false;
                    });
        ...
        // travellers.forEach((room, roomIndex) => {
            let SeqNo = 0;
            // OrderSureTravellerList = OrderSureTravellerList.concat(room);
                ...
                if(customerInfo.EmployeeDictList&&customerInfo.EmployeeDictList.length>0){
                    ...
                    for (let i = 0; i < customerInfo.EmployeeDictList.length; i++) {
                       ...
                       let itemIndex =  obj.Addition && obj.Addition.DictItemList && obj.Addition.DictItemList.find(item => {
                           ...
                           if (item.DictId == customerInfo.EmployeeDictList[i].Id) return true;
                           if (item.DictName && item.DictName == customerInfo.EmployeeDictList[i].Name) return true;
                           return false;
                       });
                       if(!itemIndex){
            ...
            travellers.forEach((obj, index) => {
                if (obj && obj.Name) {
                    if(!Util.RegEx.isRealHotelName(obj.Name)){
                    ...
                    // if(!obj.Surname){
                    //     this.toastMsg(I18nUtil.tranlateInsert('{{noun}} 英文姓不能为空', obj.Name));
                    //     alertWarning = 1
                    //     return;
                    // }
        ...
        let referencEmployeeId
        if(comp_userInfo&&comp_userInfo.employees){
            let num = comp_userInfo&&comp_userInfo.employees.length-1
            referencEmployeeId = comp_userInfo.employees[num]&&comp_userInfo.employees[num].PassengerOrigin&&comp_userInfo.employees[num].PassengerOrigin.EmployeeId
        }else{
            referencEmployeeId = userInfo.Id
        }
        const dictConfigList = customerInfo && Array.isArray(customerInfo.DictList) ? customerInfo.DictList : [];
        const intlHotelDictConfigs = dictConfigList.filter(cfg => cfg && (cfg.BusinessCategory & 64));
        const existCompanyDictItemList = AdditionInfo && Array.isArray(AdditionInfo.DictItemList) ? AdditionInfo.DictItemList : [];
        const nullCompanyDictList = intlHotelDictConfigs.map((item) => ({
            DictCode: item.Code,
            DictEnName: item.EnName,
            DictId: item.Id,
            DictName: item.Name,
        ...
        intlHotelDictConfigs.forEach((cfg) => {
            if (cfg && cfg.NextId) childIdSet.add(cfg.NextId);
        });
        const visibleCompanyIdSet = getVisibleDictIdSet(intlHotelDictConfigs, customerInfo && customerInfo.DictMapList, nullCompanyDictList);
        ...
        let params = Object.assign({ requestModel, from: 'hotel', passengerList: travellers,totalPrice:totalPrice }, this.params, this.state);
    ...
    _showPriceDetail = (merchantPrice) => {
        ...
        let paymentDesc = '';
        if (roomModel&&roomModel.PaymentType === 1) {
            if (roomModel.GuaranteeRules && roomModel.GuaranteeRules.length > 0) {
                paymentDesc = '信用卡担保';
            } else {
                paymentDesc = '前台现付';
    ...
    renderBody() {
        const { Contact, ApproveOrigin, customerInfo, userInfo, AdditionInfo, 
                LasterLiveTime,ServiceFeesData,travellers,paymentDesc, 
                paramItems,roomCount,fileList,merchantPrice,PdfDictList } = this.state;
        const { feeType ,hotelCanselRule} = this.props;
        const { roomModel } = this.params;
        ...
        return (
            <LinearGradient style={{ flex: 1, position: 'relative', }} start={{x: 1, y: 0}} end={{x: 1, y: 0.5}} colors={[Theme.theme,Theme.normalBg]}>
                ...
                <CreateOrderPriceView ref={o => this.priceView = o} {...this.params} ServiceFeesData={ServiceFeesData}  
                                      roomCount={this.state.roomCount} travellers={travellers} paymentDesc={paymentDesc}
                                      merchantPrice={merchantPrice}
                />
    ...
    _renderPayType = () => {
        ...
        return (
            <View style={{ marginTop: 10, backgroundColor: 'white', padding: 20, flexDirection: 'row',marginHorizontal: 10,borderRadius:4,justifyContent:'space-between', }}>
                ...
                <CustomText text={paymentDesc + (RcModel && RcModel.ViolationMode == 3 ? '/超标现付' : '')} style={{ flex: 7 }} />
            </View>
        )
    }
            ...
            <View style={{marginTop:10}}>
                 {
                    roomNumList&&roomNumList.map((i)=>{
                        return(
                            <View style={{ flexDirection: 'row', backgroundCoor: 'white', borderBottomColor: Theme.lineColor, borderBottomWidth: 1, backgroundColor:'#fff',marginHorizontal:10,borderRadius:6 }} >
                                <View style={{ justifyContent: 'center' }}>
                                    ...
                                    </View>
                                    {
                                        shareAllArr&&shareAllArr[i]&&shareAllArr[i].map((item,index)=>{
                                           item.RoomNumber = i+1;
                                           return (
                                              <TouchableOpacity style={{flexDirection: 'row',width:global.screenWidth-60,marginLeft:20,alignItems:'center',justifyContent:'space-between' }} onPress={()=>{
                                                this._toClickEdit(item,i,index)
                                                }}>
                                                    <View style={{flexDirection:'row',alignItems:'center'}}>
                                                        <TouchableHighlight onPress={()=>{this._deleteSharePerson(item,i,index)}} underlayColor='transparent'>
                                                            <AntDesign name={'delete'} size={16} color={Theme.theme} />
                                                        </TouchableHighlight>
    ...
    _renderTravellers = () => {
        ...
        return (
            <View style={{ marginTop: 10, backgroundColor: "white",marginHorizontal:10,borderRadius:6,paddingHorizontal: 10  }}>
                {
                    travellers&&travellers.map((item, index) => {
                        return (
                            <View key={index}>
                                <View style={{ flexDirection: 'row', backgroundColor: 'white', borderBottomColor: Theme.lineColor, borderBottomWidth: 1 }}>
                                    <View style={{ justifyContent: 'center' }}>
                                        <Text style={{ paddingHorizontal: 10 }}>{I18nUtil.translate('房间')}{parseInt(index) + 1}</Text>
                                    </View>
                                    <View style={{ flex: 1 }}>
                                        {
    ...
    _abloutLiveNumber = (data) => {
        ...
        return (
            <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 15, borderBottomWidth: 1, borderBottomColor: Theme.lineColor }}>
                ...
                {
                    <TouchableOpacity 
                        onPress={()=>{this.push('InterHotelEditPassengerScreen',{
                            customerInfo:customerInfo,
                            passenger: data,
                            IsNeedIDCard:roomModel.IsNeedIDCard,
                            // IsRewardPoint:roomModel.IsRewardPoint,
                            IsRewardPointTVP:roomModel.IsRewardPoint && (roomModel.VendorCode==='TVP' || roomModel.SubChannel==='amadeus'),
                            noComp:true,//标注不是综合订单页
                            callBack: (reason,i,num) => {
                                if(i!=1){
                                   travellers&&travellers.map((item,indexx)=>{
                                    if(item.Name === reason.Name){
                                        travellers[indexx] = reason
                                ...
                                this.setState({})
                                data = reason
                                data.CertificateNumber=num
                                data.CertificateType='身份证'
                                data.CertificateId = 1
                                }
                        }})}} 
                            style={{flexDirection:'row',width:screenWidth-115, alignItems: 'center',marginLeft:20}}>
                        <View style={{flexDirection:'column'}}> 
                            <View style={{flexDirection:'row',alignItems:'center'}}>      
                                <CustomText text={'英文名' } />
                                <CustomText text={'*'} style={{  color:'red',fontSize:24,marginLeft:2}} />
    ...
    _renderBottomView = () => {
        ...
        //判断前台现付传0 pamenttype=1前台现付
        let price = roomModel.PaymentType === 1 || roomModel.NeedCreditCard ? 0 : beforTotal//前台现付总价传0
        let merchantPrice =ServiceFeesData?.IsShowServiceFee? MerchantPriceUtil.merchantPrice( CommonEnum.orderIdentification.intlHotel, customerInfo, price, serviceP, peronalCost,this.props.feeType,needCreditCard ):0 
        // totalPrice = totalPrice + merchantPrice
        totalPrice = Number(totalPrice) + Number(merchantPrice)
        return (
            <View style={{ height: 50, flexDirection: 'row', backgroundColor: 'white', alignItems: 'center',borderTopWidth:1 ,borderColor:Theme.lineColor}}>
                <CustomText style={{ marginLeft: 10, color: Theme.theme, fontSize: 17,fontWeight:'bold' }} text={'CNY' + Number(totalPrice).toFixed(2)} />
...
export default connect(getStatePorps, mapDispatchToProps)(IntelHotelCreateOrderScreen);
