...
import IntlHotCities from '../../res/js/intlHotel_hotCities';
export default class IntlHotelCityScreen extends SuperView {
    ...
    componentDidMount() {
        InteractionManager.runAfterInteractions(() => {
            StorageUtil.loadKeyId(Key.IntlHotelCitysData).then(response => {
                if (response) {
    ...
    _loadCityData = () => {
        ...
        IntlHotelService.cityList().then(response => {
            
            this.hideLoadingView();
            if (response && response.success) {
                StorageUtil.saveKeyId(Key.IntlHotelCitysData, response.data);
    ...
    _analyData = (data) => {
        ...
        data.forEach(item => {
            ...
            const section = sections.find(obj => obj.title === firstChar);
            if (section) {
            ...
            this.state.recordSections.forEach(item => {
                if (item.data) {
                    item.data.forEach(obj => {
                        ...
                        let miniLetters = []
                        arrLetters&&arrLetters.map(item=>{
                            miniLetters.push(item.toLowerCase())
                        })
                        if (obj.Name && obj.Name.includes(this.state.keyWord) || (obj.EnName && obj.EnName.includes(this.state.keyWord))||
                            arrLetters&&arrLetters.includes(this.state.keyWord) || miniLetters&&miniLetters.includes(this.state.keyWord.toLowerCase()) ||
                            (obj.EnName && obj.EnName.toLowerCase().includes(this.state.keyWord.toLowerCase()))
                        ) {
                           list.push(obj);
                        }
                    })
                                        ...
                                        return (
                                            <TouchableHighlight key={index} underlayColor="transparent" onPress={this._cityBtnClick.bind(this, item)}>
                                                <View style={{
                                                    ...
                                                    <CustomText numberOfLines={1} text={Util.Parse.isChinese() ? item.Name : item.EnName}  style={{fontSize:12, color:Theme.commonFontColor}}/>
                                                </View>
                                            </TouchableHighlight>
                                        )
                                    })
                                        ...
                                        return (
                                            <TouchableHighlight key={index} underlayColor="transparent" onPress={this._letterBtnClick.bind(this, item)}>
                                                <View style={{
                                                    ...
                                                    <CustomText numberOfLines={1} text={item} />
                                                </View>
                                            </TouchableHighlight>
                                        )
                                    })
        ...
        return (
            <TouchableHighlight underlayColor='transparent' onPress={this._cityBtnClick.bind(this, item)}>
                <View style={{
                    ...
                    <CustomText style={{ }} text={Util.Parse.isChinese() ? item.Name : item.EnName} />
                    <CustomText style={{ marginLeft: 15, fontSize:13,color:'gray' }} text={ Util.Parse.isChinese() ? item.NationalName : item.EnNationalName} />
                </View>
            </TouchableHighlight>
        )
    }
