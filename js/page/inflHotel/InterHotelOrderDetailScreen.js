...
class InterHotelOrderDetailScreen extends SuperView {
    constructor(props) {
        super(props);
    ...
    _priceInfo = (order) => {
        ...
        return (
            <View style={{ backgroundColor: '#fff',marginHorizontal:10,marginTop:10,paddingHorizontal:20,paddingVertical:10,borderRadius:6}}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', borderBottomColor: Theme.themeLine, borderBottomWidth: 1 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center',paddingVertical:10 }}>
                    <TextViewTitle title={'应付金额'} style={{marginLeft:-1,}} imgIcon={require('../../res/Uimage/shu.png')}/>
                        <CustomText text={'￥'+order.Amount} style={{ color: Theme.fontColor,fontSize:14 }} />
                        {/* <Text style={{fontSize:12, color:Theme.specialColor,marginTop:2}}>({I18nUtil.translate(order.PaymentType)})</Text> */}
                        {
                             order.RatePlan.ChannelTag ?
                             <View style={{ backgroundColor: Theme.orangelableColor, alignItems: 'center',marginHorizontal,flexDirection:'row',borderRadius:2,paddingHorizontal:6 }}>
                ...
                {
                        <View style={{ backgroundColor: 'white', paddingVertical: 10,borderRadius:6 }}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <CustomText style={{ color: Theme.commonFontColor, fontSize: 14 }} text='公司支付' />
                                <CustomText style={{ color: Theme.fontColor, fontSize: 14 }} text={'¥' + order.CompanyAmount} />
                            </View>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between',paddingTop:5 }}>
                                <CustomText style={{ color: Theme.commonFontColor, fontSize: 14 }} text='个人支付' />
                                <CustomText style={{ color: Theme.fontColor, fontSize: 14 }} text={'¥' + order.PersonalAmount} />
                            </View>
                            {
                                IsShowServiceFee?
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between',paddingTop:5 }}>
                                    {/* <CustomText style={{ color: Theme.specialColor, fontSize: 13 }} text='服务费' /> */}
                                    <View style={{
                                            flexDirection: 'row',
                                            alignItems: "center"
                                        }}>
                                            <CustomText style={{ color: Theme.commonFontColor, fontSize: 14 }} text='服务费' />
                                            {
                                                order.ServiceCharge > 0 ?
                                                    <AntDesign name={'questioncircle'} color={Theme.theme} size={17} style={{ marginHorizontal: 5 }} onPress={this.showServiceInfo} />
                                                    : null
                                            }
                                        </View>
                                    <CustomText style={{color: Theme.fontColor, fontSize: 13 }} text={'¥' + order.ServiceCharge} />
                                </View>:null
                            }
                ...
                {
                    order.RefundInfoList && order.RefundInfoList.length > 0 ?
                        <View style={{ marginVertical: 10 }}>
                            <TouchableHighlight underlayColor='transparent' onPress={() => this.setState({ isShowDetailPrice: !this.state.isShowDetailPrice })}>
                                <View style={{ height: 40, flexDirection: 'row', alignItems: 'center', backgroundColor: 'white', justifyContent: 'space-between', borderBottomColor: Theme.lineColor, padding: 10, borderBottomWidth: 0.5,borderRadius:6 }}>
                    ...
                    order.OrderHotelReasons&&order.OrderHotelReasons.map((item)=>{
                        return(
                            < Text style={{ flexDirection: 'row', flex: 1, alignItems: 'center',marginTop:5 }}>
                                ...
                                <CustomText style={{ color: Theme.annotatedFontColor, fontSize: 12, flex: 1 }}  text={'('+item.RuleTypeDesc+')'+(Util.Parse.isChinese()? item.Reason: item.ReasonEn)} />
                            </Text> 
                        )
                    })
                }
                {
                    order.ExcessAmount>0?
                        ...
                        <View style={{ flexDirection: 'row', flex: 1, alignItems: 'center',marginTop:5}}>
                            <Text style={{ flex: 1, fontSize: 12, color:Theme.orangelableColor }}>{ExcessStr1}</Text>
                        </View>
                        :
                        <View style={{ flexDirection: 'row', flex: 1, alignItems: 'center',marginTop:5}}>
                            <Text style={{ flex: 1, fontSize: 12, color:Theme.orangelableColor }}>{ExcessStr2}</Text>
                        </View> 
                    :null 
                }
            </View>
    ...
    */
    ...
    _renderApproveBtn = (order) => {
        if (this.params.isApprove && order.Status === 17) {
            return (
        ...
        */
        ...
        return (
            <View style={{backgroundColor:'#fff',marginHorizontal:10,borderRadius:6,paddingHorizontal:20,paddingVertical:20 }}>
                <CustomText style={{  fontSize:15, fontWeight:'bold'}} numberOfLines={1} text={Util.Parse.isChinese() ? result.Hotel.Name : result.Hotel.EnName} />
                <CustomText style={{  fontSize:12,marginTop:5}} text={result.Hotel.Address} />
                ...
                <Text allowFontScaling={false} style={{ fontSize: 13 ,color:Theme.fontColor,fontWeight:'bold',marginVertical:5}}>{result.Room.RoomName + ' ' + (result.Room.BedType ? result.Room.BedType : '') + ' ' + haveBreackFast}</Text>
                <Text>
                ...
                {/* <Text allowFontScaling={false} style={{marginTop: 5, fontSize: 13,fontWeight:'bold',color:Theme.fontColor }}>{I18nUtil.translate('取消规则')}</Text> */}
                <TextViewTitle title={'取消规则'} style={{marginLeft:-1,}} imgIcon={require('../../res/Uimage/shu.png')}/>
                <Text allowFontScaling={false} style={{marginTop: 5, fontSize: 12,color:Theme.commonFontColor }}>{I18nUtil.translate(cancleRuleS)}</Text>
    ...
    renderBody() {
        const { order, customerInfo, options } = this.state;
        if (!order) return;
        return (
...
export default connect(getStatePorps)(InterHotelOrderDetailScreen);